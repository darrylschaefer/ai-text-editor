...
const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  return (
     ...
     >
     <div 
     className='p-6 border-b border-gray-200 dark:border-gray-800/30 w-full'>
        <div className='flex mb-6 flex-col space-y-4'>
            <div>
            <label
            className='block text-sm font-medium text-gray-900 dark:text-white mb-2'>Name:</label>


            <textarea
                  className='m-0 resize-none rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-800/30 mt-2 p-2 text-gray-900 dark:text-gray-400 overflow-y-hidden leading-7 p-1 focus:ring-1 focus:ring-blue focus:border-blue-500 dark:focus:border-blue-400 w-full max-h-10 transition-all'
...
<div>
  ...
  {currentApiEndpoint === 'completions' ? (
                <div className="relative w-full">
                  <StyledTextarea
                    className='m-0 resize-none rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-800/30 mt-2 p-2 text-gray-900 dark:text-gray-400 leading-7 p-1 focus:ring-1 focus:ring-blue focus:border-blue-500 dark:focus:border-blue-400 w-full transition-all'
                    onChange={(value) => {
    ...
    <>
      ...
      >
        + Convert to prompt list
                ...
                <div className="pt-2">
                  <label className='block text-sm font-medium text-gray-900 dark:text-white mb-2'>Config Source:</label>
                  <select
                    className='w-full pl-3 pr-8 py-2 border border-gray-300 dark:border-gray-700/40 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100'
        ...
        />
          ...
          <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Using {defaultConfigType === 'agent' ? 'Agent' : defaultConfigType === 'legacy_chat' ? 'Legacy Chat Completions' : 'Legacy Completions'} defaults. 
...
const PromptToolSelection = ({
  ...
  useEffect(() => {
    // Get fresh prompts from store to avoid stale closure
  ...
  }, [useCustomTools, _enabledTools, _toolConfirmationSettings, index, setPrompts]);
